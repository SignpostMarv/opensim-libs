...
namespace HttpServer.Test
{
    public class HttpListenerTest
    {
        ...
        {
            HttpListener listener = new HttpListener(IPAddress.Any, 8321);
            listener.RequestHandler += OnIncomingRequest;
            listener.Start(20);
        ...
        public void TestSequential()
        {
            ...
            {
                _testEvent.Reset();
                HttpListener listener = new HttpListener(IPAddress.Any, 4324);
                listener.RequestHandler += OnIncomingRequest;
                listener.Start(5);
        ...
        private void OnTestStartStop(object obj)
        {
            ...
            {
                WebClient _client = new WebClient();
                Assert.Equal("Hello world!", _client.DownloadString("http://localhost:8321/welcome/to/tomorrow/"));
                Assert.Equal("Hello world!", _client.DownloadString("http://localhost:8321/welcome/to/tomorrow/"));
            }
            catch(WebException){}
        ...
        private void OnIncomingRequest(IHttpClientContext client, IHttpRequest request)
        {
