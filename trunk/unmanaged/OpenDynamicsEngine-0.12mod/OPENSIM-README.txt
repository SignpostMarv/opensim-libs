...
The ODE code in this repository correspondes to ODE release 0.12 plus some selected patchs from newer releases 

= opcode versus gimpact =
gimpact mesh collider on ODE is a older version of what is now part of Bullet.
It can provide better results in some cases, usually with higher cpu load.
Also notice fails in collision detection of some faces.
So opcode is still the best option for ODE.
If Gimpact performance is needed then Bullet engine is a option.

= libccd =
this option should provide suport for convex objects.
Some testing didn't provide good results, so not in use even in ubOde
...
for windows 32bits target:
	premake4 --only-single --platform=x32 vs2008	
for windows 64bits target:
	premake4 --only-single --platform=x64 vs2008	
...
CFLAGS=-m32 CPPFLAGS=-m32 LDFLAGS=-m32 ./configure --build=i686-pc-linux-gnu --with-trimesh=opcode --disable-asserts --enable-shared --disable-demos --without-x
make
...
CFLAGS="-g -O2 -isysroot /Developer/SDKs/MacOSX10.6.sdk -arch i386 -arch x86_64 -arch ppc" CXXFLAGS="-g -O2 -isysroot /Developer/SDKs/MacOSX10.6.sdk -arch i386 -arch x86_64 -arch ppc" LDFLAGS="-arch i386 -arch x86_64 -arch ppc" ./configure --with-trimesh=opcode --disable-asserts --enable-shared --disable-dependency-tracking --disable-demos --without-x
make
...
[ubODE] ode library configuration: ODE ODE_EXT_trimesh ODE_EXT_opcode ODE_OPC_new_collider ODE_single_precision
